...
# node_exporter
git clone https://$github/JohnsonRan/packages_utils_node-exporter package/new/node-exporter

# sysupgrade keep files
echo "/etc/hotplug.d/iface/*.sh" >>files/etc/sysupgrade.conf
echo "/etc/mihomo/run/cache.db" >>files/etc/sysupgrade.conf
...
sed -i 's|raw.githubusercontent.com|raw.e11z.net/raw.githubusercontent.com|g' package/new/default-settings/default/zzz-default-settings
# hey TUNA
