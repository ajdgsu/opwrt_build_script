...
mkdir -p files/etc/ak_monitor
ak_version=$(curl -s https://api.github.com/repos/akile-network/akile_monitor/releases/latest | grep tag_name | cut -d '"' -f 4 | sed 's/^v//')
if [ "$platform" = "rk3568" ]; then
    curl -skLo ne.tar.gz https://$github/akile-network/akile_monitor/releases/download/v$ak_version/akile_monitor-linux-arm64
    chmod +x akile_monitor-linux-arm64
    mv akile_monitor-linux-arm64 files/etc/ak_monitor/ak_client
else
    curl -skLo ne.tar.gz https://$github/prometheus/node_exporter/releases/download/v$ak_version/akile_monitor-linux-amd64
    chmod +x akile_monitor-linux-amd64
    mv akile_monitor-linux-amd64 files/etc/ak_monitor/ak_client
fi
curl -skLo files/etc/init.d/ak_monitor  $mirror/openwrt/files/etc/ak_monitor/ak_monitor.init
curl -skLo files/etc/ak_monitor/client.json  $mirror/openwrt/files/etc/ak_monitor/client.json
